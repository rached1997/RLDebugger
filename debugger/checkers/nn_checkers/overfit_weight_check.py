...
class OverfitWeightsCheck(DebuggerInterface):
    def __init__(self):
        super().__init__(check_type="OverfitWeight", config=get_config())
        self.error_msg = list()
    ...
    def check_sign(self, weight_name, weight_array, is_conv):
        if self.config['neg']['disabled']:
            return
        neg_ratio = (weight_array < 0.).sum().item() / weight_array.size
    ...
    def check_dead(self, weight_name, weight_array, is_conv):
        ...
        dead_ratio = torch.sum(
            (torch.abs(weight_array) < self.config['dead']['val_min_thresh']).int()).item() / weight_array.size
