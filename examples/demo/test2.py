...
def optimize_model():
    ...
    rl_debugger.debug(
        ...
        target_model_update_period=target_model_update_period,
        target_net_update_fraction=TAU,
        loss_fn=criterion,
        opt=optimizer,
        targets=expected_state_action_values.unsqueeze(1).detach(),
        training_observations=state_batch,
        discount_rate=GAMMA,
        predicted_next_vals=next_state_values.detach(),
...
def select_action(state):
    ...
    rl_debugger.debug(
        actions_probs=action_probs.detach(), exploration_factor=eps_threshold)
    steps_done += 1
...
rl_debugger.set_config(config_path="dqn_example.yml")
